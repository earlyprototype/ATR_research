...
> **Addendum, 2026-07-31 (end of day).** This review describes the repository as it stood at `main` = `eb86404` on the morning of 2026-07-31; the body below is a dated record and has not been rewritten. Since it was written, several of its recommendations have been executed. Pull request #10 (the 300-window census, §3.1 row 11) was merged by the operator (merge commit `a34f1d9`), and §6.2 items 2 and 3 landed on `main` via pull request #57 (merge commit `02cd84f`): `_STAGE2_JSPACE/REGISTER.md` now exists as the authoritative identifier register, the deleted 2026-07-24 permutation run described in §4.2 is restored in-tree (erratum section in `RESULTS_EXP010C.md` plus the artifact `permutation_results_2026-07-24_seed2026.json`), the `RESULTS_EXP010B.md` numbers from §4.4 item 1 are corrected against their artifact, `PERM_TEST_EXP010c_SPEC.md` carries its SUPERSEDED header, the discussion #37 "H11a vacated" error from §4.1 is corrected in the register's errata, and the `SKILL.md` over-count from §4.4 item 6 is fixed. Issue #52 is closed. Issue #59 now charters the natural-energy comparison test that §3.3 step 1 called for (spec-first, analysis-only). Still open as of this addendum: the §6.2 item 4 code fixes (the overwrite guard still omits `--arms`, the BOS check of §4.4 item 3 has not run, and the permutation attestation of §4.4 item 4 was not regenerated), the governance file and CI of §5.3 and §6.2 item 5, the kill-criteria re-registration of item 6, the housekeeping sweep of item 7 (issues #11 through #14 remain open and 20 stale `claude/*` branches remain on the remote), the re-charter of issues #45 and #46 from item 8, and the PR #53 disposition of §6.3. A plain-language companion written for the operator, with every carried number re-verified against the committed artifacts on this date, is `OPERATOR_REPORT_2026-07-31.md` at the repository root; the twelve source reports behind this review are committed under `_REVIEW_2026-07-31/`.
...
All locations below cite the repository state at `main` = `eb86404`; items marked *(since fixed)* were corrected on main by PR #57 (merge `02cd84f`).

1. **`_STAGE2_JSPACE/experiments/exp_010b_small/RESULTS_EXP010B.md:45, 52–54`** *(since fixed)* — five numbers (SB basin-size tuple; off-diag cosine minima for SB/S2/S3/S5) match no committed artifact; `_STAGE2_JSPACE/experiments/exp_010b_small/output/terminal_characterisation_small010b.json` says [6,5,4,3,3,3,1] and 0.6921/0.4983/0.97/0.441. Not verdict-bearing; still a breach of "committed artifact = authority."
2. **`_STAGE2_JSPACE/experiments/exp_010c_windows/run_exp010c.py:352–366`** — the artifact-overwrite guard still omits `--arms` from the variant list: a bare `--tier full --arms A5` run would silently replace the registered 150-record `results_full.json`. A `--tag` value colliding with a tier name is also unchecked. This is the exact failure mode the guard was built for.
3. **BOS seam** *(suspected — the one item in this list pending verification)* — `_STAGE2_JSPACE/RESULTS_JLENS_MEDIUM.md:293` claims "the ATR engine does not prepend BOS"; the engine passes raw strings to `run_with_cache` and transformer_lens defaults `default_prepend_bos=True` for GPT-2, so the note is probably **backwards**. Classify as a confirmed defect only once the one-line check against pinned TL 3.5.1 is run and documented; either way the seam must be settled before any lens-on-terminals work (EXP_011m/EXP_013m).
4. **`_STAGE2_JSPACE/experiments/exp_010c_windows/output/permutation_results.json`** lacks the `input_sha256` attestation field its own spec addendum describes — never regenerated after the attestation code landed.
5. **Discussion #37 registry table** *(since fixed — corrected in REGISTER.md's errata and by board dispatch to #37)* — "H11a vacated" is wrong (see 4.1); H10-series, H14, EXP_014, and every live EXP ID are unregistered; the registry has never once been used prospectively.
6. **`.claude/skills/peer-board/SKILL.md:120`** *(since fixed)* — "H11 was claimed three separate ways" is a verified over-count (two claims — PRs #5 and #10 — plus PR #20/#33's explicit *deferral*). Every agent loads this skill; its founding incident should be stated correctly.
7. **Stale caveats** — EXP_010d's rescued record (`_STAGE2_JSPACE/experiments/exp_010c_windows/RESULTS_EXP010D.md`) carries "single seed" (vacuous since 07-25; should read "single prompt subset") and no stopping-rule caveat on its terminal partitions; H9/H10a/H10b verdicts phrased against "the collapse" were never re-scoped after the collapse was shown apparatus-conditional.
