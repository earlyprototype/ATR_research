...
processor thread when it was first run, and 213 seconds when it was re-run on
2026-09-06, the difference being other jobs sharing the processor rather than
anything about the work, so recomputing everything in the consistent frame was
cheap and was run rather than estimated. It took 114 seconds and wrote
...
18. **Neither a fit checkpoint nor a finished lens file said which model or
    ...
    **What the committed artifacts establish about themselves, exactly.** Both
    ...
    committed files. **Added 2026-09-06:** the committed
    `output/exp017_jspace.json` did not carry that field when this paragraph
    was written, because the artifact predated it, and a reviewer was right to
    say so. The artifact has since been regenerated by the documented command
    with every result field identical, so the field is now in the file it
    describes; deviation 22 records that regeneration and everything that
...
21. **Four more gates failed open rather than closed, and one table generator
    could crash.** A third review on 2026-09-06 found them. None changes a
    number; all four are about what a later run in a damaged or unusual tree
    would be allowed to do. First, when `output/fit_budget_decision.json` could
    not be read, the prompt-count requirement became None and every lens passed
    it, so a tree missing its own budget artifact could have emitted a bare
    H18b verdict. A missing requirement now refuses any twin lens and takes the
    spec's section 6.2 route, and the run is marked as not the registered
    comparison, which also means it may not write the registered filename.
    Second, `choose_fit_budget.py` took the largest multiple of ten that fits
    inside the 9,000 second cap but never let that be smaller than ten, so a
    probe measuring more than 900 seconds a prompt would have recorded that ten
    prompts fit when they cost 9,000 seconds or more. That case now records a
    chosen count of zero and says in the decision's own words that no fit fits
    and the section 6.2 fallback applies, and `run_jspace.py` reads a count of
    zero exactly that way. **The committed decision is untouched and that is
    checked, not assumed:** re-running the rule on the committed probe log in a
    scratch directory reproduces `output/fit_budget_decision.json` byte for
    byte, 221.0 seconds a prompt and 40 prompts. Third, a checkpoint whose fit
    the clock had stopped could be resumed by the same command with a fresh
    9,000 second budget, so the resumed fit would have measured only its own
    hours, finished as complete, and stamped a lens whose true cost was the sum
    of two runs. **The choice made, and why:** such a checkpoint is now refused
    on both the seeding path and the resume path rather than having its elapsed
    time carried forward, because spec section 6.2 already says what a fit the
    cap stops means, which is that H18b is scored with the base lens on both
    sides, and because a deliberately longer fit remains possible under its own
    tag and its own `--deadline-seconds`, which stamps the lens with that longer
    cap, which the probe then refuses to score as registered. The committed
    five-prompt probe checkpoint is unaffected: it carries no sidecar at all, so
    it is still handled as unstamped and still needs
    `--accept-unstamped-checkpoint`. Fourth, `make_tables.py` would have raised
    an error rather than a table if the Hugging Face frame artifact existed
    without the registered one; it now renders whichever sections it has, which
    was checked on copies in a scratch directory for all three combinations, and
    the default run still reproduces `output/tables.md` with no line changed.

22. **The registered J-space artifact was regenerated on 2026-09-06, with
    identical numbers, so that it carries the provenance fields the record
    describes.** The same review noticed that this record said the explicit
    acceptance of the unstamped twin lens is recorded in the output artifact,
    while the committed `output/exp017_jspace.json`, written on 2026-09-05,
    carried neither `twin_lens_provenance` nor `twin_lens_budget` nor the frame
    stamp, because it predated all of them. The documented command was re-run
    with the current code and its output compared with the committed file field
    by field. **Every result field is identical:** every one of the 4,400 share
    values across both lenses, both models and the four dictionary variants,
    every control spread, every permutation p, every cross-check, every
    model-effect and instrument-effect number, every lens diagnostic, the
    rescale factors, and the verdict string SUPPORTED with band layers 6, 7, 9
    and 10. Exactly three things differ and all three are named here. The
    `wall_seconds` field reads 213.4 where it read 159.5, because it measures
    this machine's clock on the day rather than a result; **withdrawn by name:
    the 160 seconds this record quoted for the probe's cost, which is now 213
    seconds**, and both figures are for the same work on the same machine, with
    the difference being other jobs sharing the processor. The two lens `path`
    fields now hold the paths the documented command passes, the checkout
    relative `../../artifacts/jlens_lamini_gpt2_124m_40_twin.pt` and this
    worktree's own `_STAGE2_JSPACE/artifacts/jlens_gpt2_small_neuronpedia.pt`,
    where the file written on 2026-09-05 held one absolute path from the machine
    that ran it, which was itself a problem an earlier review raised. Thirteen
    fields are added, all of them stamps: the coordinate frame and its note, the
    budget decision and its source, the twin lens provenance decision with the
    acceptance flag recorded, and the empty list of reasons this run is not the
    registered comparison. **The file this replaces is not lost:** it stands in
    this branch's git history at commit `1effbab` and earlier, and the
...
6. `python3 run_jspace.py --twin-lens
   ...
   --accept-unstamped-twin-lens`, the J-space probe, 213 seconds, that is under
   4 minutes (the `wall_seconds` field of `output/exp017_jspace.json`).
   **Correction, 2026-09-06, withdrawing the 160 seconds this step used to
   quote:** that was the first run's clock, and the artifact now committed is
   the 2026-09-06 re-run of the same command, whose every result field is
   identical and whose wall time was 213 seconds. Deviation 22 records the
   comparison and what else changed in the file. The
