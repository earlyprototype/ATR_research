...
| H17 | EXP_016 | Report swap on base GPT-2 Small: exchanging the lens coordinates of the model's own top concept and a target concept (the paper's patching in lens coordinates, at one layer or a small layer set) puts the target in the next-token top-5 on at least 50 percent of trials, against at most 10 percent for norm-matched random-direction swaps | **SUPPORTED** (held-out half 86 percent, 18 of 21, against 0 percent, 0 of 42, at the tuned setting layers 7-8-9, strength 2, all positions, source rule output, with the rule part of the tuned selection as the specification's section 5.1 states; pooled over both source rules 64 percent, 27 of 42, against 0 of 84; both readings clear the registered thresholds; the target word was in the model's top five on 0 of 84 items before any swap, by the battery's construction; the one probability offered for this row comes from a test that multiplies nothing and assumes nothing about whether the frames are independent of one another: the 84 frames are split into halves that share no concept, which is possible because they fall into four disjoint category word lists, the whole setting is chosen on one half and scored on the other, and the lens arm's held-out total is compared with 499 random assignments of directions to concepts, in each of which a whole half is scored under one assignment and the control chooses its own target concept by the same layer-8 rule the battery used. Under that split the setting chosen is layers 6-7-8, strength 2.0, all positions, source rule output, the lens arm redirected 20 of 22 held-out frames against 1 of 44 for control A, and 0 of 499 random assignments reached the lens arm's total, giving 0.002, which is the floor 499 draws impose; the best single random assignment redirected 7 of 22 frames. Under the split the specification registers, whose two halves share 19 of the battery's 36 concepts so that the setting was not chosen independently of the outcomes it is scored on, the same test gives 0.002 on 21 held-out frames, see corrections 29 and 30 of the results record. Four earlier figures are withdrawn as headlines: the item-level 2.6 times 10 to the power minus 9, see correction 16; 5.1 times 10 to the power minus 5 against control A, see correction 19; 5.1 times 10 to the power minus 5 with the source concept as the group, see correction 25; and 0.00137 with the connected component as the group, which multiplies a factor per group, see correction 30) | `experiments/exp_016_swaps_small/RESULTS_EXP016.md` |
...
| H17b | EXP_016 | Intermediate-step surgery: on two-hop completions the clean model answers correctly, swapping the intermediate concept changes the final answer to the predicted alternative more often than the random-direction control | **SUPPORTED on the registered wording, weakly** (4 of 16 items against 0 of 16 for control A over three draws each; exact within-item probability 0.25 on the held-out half, 1 of 8 against 0 of 8, the only valid test of the selected setting since the tuning half chose it, and that test cannot distinguish the effect from chance; on the specification's top-three clean gate, and all four flips lie among the 7 items the clean model answers at rank one, 4 of 7 against 0 of 7; no two of the eight held-out items share a concept, so the cluster-level test that changed the H17 and H17a probabilities returns the same 0.25 here, whether the groups are formed by source concept or as connected components of the items sharing any direction, and it needs no cluster-matched control because a group of one item is the within-item test already; 0.25 is also the smallest probability that test can return on these outcomes, since seven of the eight held-out items have a lens draw and three control draws that all failed, see correction 20 of the results record; a second reading of the same eight items, which scores the whole held-out set under each of 499 random assignments of directions to concepts rather than conditioning on each item alone, gives 0.002, because not one of those 499 assignments flipped any of the eight items while the lens arm flipped one, and it carries the caveat that 3 of this battery's 16 concepts appear in both of its halves so its setting was not chosen independently of what it is scored on, see corrections 29 and 30; which of the two readings the row should carry is decision item 2 for the operator; a second reading of the same eight items, which scores the whole held-out set under each of 499 random assignments of directions to concepts rather than conditioning on each item alone, gives 0.002, because not one of those 499 assignments flipped any of the eight items while the lens arm flipped one, and it carries the caveat that 3 of this battery's 16 concepts appear in both of its halves so its setting was not chosen independently of what it is scored on, see corrections 29 and 30; which of the two readings the row should carry is decision item 2 for the operator; over all 16 items, which is a post-selection reading and no test of the chosen setting, three pairs of items do share a concept and the figure moves from 0.0039 to 0.0156, see correction 25). Owner ruling invited: see decision items 2 and 5 of the results record | same record |
