"""Correctness checks on the swap engine, run before the batteries. Each
check asserts its invariant and the script exits non-zero if any fails.
...
d1 = (out - torch.log_softmax(clean, -1)).abs().max().item()
print("(1) swapping a token with itself, max change in log-probabilities:", f"{d1:.2e}")
assert d1 < 1e-4, f"identity swap changed the output by {d1:.2e}"
...
assert torch.allclose(newc, c.flip(0), rtol=1e-3, atol=1e-3 * c.abs().max()), \
    "coordinates were not exchanged"
...
assert resid.norm().item() < 1e-3 * patch.norm().item(), "change left the plane"
...
d4 = (a - b[0]).abs().max().item()
print("(4) batched against single, max difference:",
      f"{d4:.2e}; the size-matched control row "
      f"differs from the lens row by {(a - b[5]).abs().max().item():.3f}")
assert d4 < 1e-4, f"batched and single paths differ by {d4:.2e}"
assert (a - b[5]).abs().max().item() > 1e-3, "the control row did not differ from the lens row"
...
print("all four engine checks passed")
