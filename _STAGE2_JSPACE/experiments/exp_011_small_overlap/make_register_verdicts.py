...
NOTES = {
    ...
    "H16a": ("prolet beats the trace injected from phase A at three of six band "
             "layers and the trace injected from phase B at one of six, where four of "
             "six against each was required. The verdict does not depend on which "
             "trace carries which phase name, because the scoring rule is symmetric "
             "in the two phases. Finding F16 is not overturned. At layer 11, the only "
             "layer where this experiment and F16 measure the same thing, namely a "
             "single vector scored against the final layer's dictionary, the "
             "full-vocabulary lens reproduces F16's ordering: phase A 0.1516, above "
             "prolet 0.0912, above phase B 0.0164, on a 0-to-1 share scale. At the "
             "band layers this experiment measures intermediate residuals of one loop "
             "step rather than the phase vectors F16 probed, so it neither confirms "
             "nor refutes F16 there. An earlier version of this row claimed a "
             "retraction of F16's phase assignment; that claim was wrong and is "
             "withdrawn, see the correction dated 2026-09-05 in the results record"),
